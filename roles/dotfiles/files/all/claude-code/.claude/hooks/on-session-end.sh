...
if [ -n "$SESSION_ID" ] && [ "$SESSION_ID" != "null" ]; then
    ...
    log_debug "Removed /tmp/claude_${SESSION_ID}_{pane,repo,started,completed,waiting,done}"
fi
...
find /tmp -maxdepth 1 -name 'claude_*_done' -mtime +7 -delete 2>/dev/null
log_debug "Swept stale claude_*_{pane,repo,started,completed,waiting,done} files older than 7 days"
